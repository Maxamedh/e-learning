    ...
    <div class="row g-0">
        ...
        <div class="col-lg-8 video-player-container">
            <div class="video-wrapper">
                <?php if (!empty($defaultVideo)): ?>
                    ...
                    <?php if (ENVIRONMENT === 'development'): ?>
                        <div style="position: absolute; top: 10px; left: 10px; background: rgba(0,0,0,0.7); color: #fff; padding: 5px 10px; font-size: 0.8rem; z-index: 1000; border-radius: 3px;">
                            Video URL: <?= esc($defaultVideo) ?>
                        </div>
                    <?php endif; ?>
                    <video id="courseVideoPlayer" controls controlsList="nodownload" preload="metadata" crossorigin="anonymous" style="width: 100%; height: 100%;">
            ...
            <div class="video-info">
                <h4 id="currentLectureTitle"><?= !empty($defaultLecture) ? esc($defaultLecture['title']) : esc($course['title']) ?></h4>
            ...
            <div class="course-info-panel">
                ...
                <div class="course-details">
                    <div class="detail-item">
                        ...
                        <span><strong>Instructor:</strong> <?= esc(($instructor['first_name'] ?? '') . ' ' . ($instructor['last_name'] ?? 'N/A')) ?></span>
                    </div>
                    <div class="detail-item">
                ...
                <?php else: ?>
                    <?php foreach ($sections as $sectionIndex => $section): ?>
                        <div class="section-item">
                            <div class="section-header <?= $sectionIndex === 0 ? 'active' : '' ?>" onclick="toggleSection(<?= $section['id'] ?>)">
                                <div>
                                    <span class="section-title"><?= esc($section['title']) ?></span>
                                    <span class="section-info">
                                        (<?= count($section['lectures']) ?> lecture<?= count($section['lectures']) != 1 ? 's' : '' ?>)
                                    </span>
                                </div>
                                <i class="fa-solid fa-chevron-right section-toggle <?= $sectionIndex === 0 ? 'expanded' : '' ?>" id="toggle-<?= $section['id'] ?>"></i>
                            </div>
                                    ...
                                    <?php foreach ($section['lectures'] as $lectureIndex => $lecture): ?>
                                        <div class="lecture-item <?= ($sectionIndex === 0 && $lectureIndex === 0 && !empty($defaultLecture) && $defaultLecture['id'] == $lecture['id']) ? 'active' : '' ?>" 
                                            ...
                                            <div class="lecture-title">
                                                <i class="fa-solid fa-<?= $lecture['content_type'] === 'video' ? 'play-circle' : ($lecture['content_type'] === 'article' ? 'file-alt' : 'question-circle') ?> lecture-icon"></i>
...
<script>
    ...
    if (videoPlayer) {
        videoPlayer.addEventListener('error', function(e) {
            ...
            errorMsg.innerHTML = '<i class="fa-solid fa-exclamation-triangle fa-3x mb-3"></i><p>' + errorMessage + '</p><small style="color: #ccc; font-size: 0.8rem;">Check browser console for details</small>';
